...
while True:
    take_species = input(
        "Please enter the species names for the transcript map")
...
for i in range(len(fasta_names)):
    start.append(list(filter(lambda n: n != "-",
                 re.findall(r'[0-9]{0,3}-[0-9]{0,6}', fasta_names[i])))[0].split("-")[0])
    end.append(list(filter(lambda n: n != "-",
               re.findall(r'[0-9]{0,3}-[0-9]{0,6}', fasta_names[i])))[0].split("-")[1])
...
with open("save_coordinates_visualization.txt", "w") as write_gff:
    write_gff.write(
        f"The coordinates for the transcript structure are given below from the transcriptome assembly")
    write_gff.write("\n")
